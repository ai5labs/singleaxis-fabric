...
def test_v1_open_when_token_unset(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with TestClient(app) as c:
        ...
        resp = c.post("/v1/redact", json=_REDACT)
...
def test_v1_rejects_missing_and_wrong_token(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with TestClient(app) as c:
        # No header at all.
        assert c.post("/v1/redact", json=_REDACT).status_code == 401
        # Wrong value — including a prefix of the real token.
        assert (
            c.post("/v1/redact", json=_REDACT, headers={"X-Fabric-Token": "nope"}).status_code
            == 401
        )
        assert (
...
def test_v1_accepts_matching_token(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    app = build_app(tenant_key=b"t")
    with TestClient(app) as c:
        resp = c.post("/v1/redact", json=_REDACT, headers={"X-Fabric-Token": "s3cret"})
        assert resp.status_code == 200
        assert "value" in resp.json()
